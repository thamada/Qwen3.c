...
#### 実測（RADV GFX1201・`Qwen_Qwen3-VL-8B-Instruct-IQ2_M.gguf`・132+16 トークン・2026-05-29 09:39）

| 指標 | Vulkan | ROCm（同日 08:12） |
|------|-------:|------------------:|
| prefill tok/s | **2.97** | **124.95** |
| decode tok/s | **2.17** | **28.88** |
| total tok/s | **2.86** | **91.89** |

#### `qwen3-8b/gpu-vulkan/Makefile`

- **`GPU_VK`**: **`vulkaninfo`** の **`deviceName=`** を **`awk -F'='`** で正しく抽出（旧実装は第 2 フィールドが **`=`** のみで **`BENCH_LOG`** 第 2 列が壊れていた）。

#### ドキュメント

**`README.md`** / **`README.en.md`**: **Vulkan GPU 長プロンプト**節（実測表・VRAM 内訳・ROCm 比較）を追加。短プロンプト参考表を整理。

**`doc/design.md`**: バリアント表・Vulkan ビルド節・実装メモのベンチ数値・VRAM 説明を同期。
